...
public class SeacherService {
    ...
    private SearchResponseVo parseResult(SearchResponse searchResponse){
        ...
        if (!CollectionUtils.isEmpty(buckets)){
            List<BrandEntity> brandEntities = buckets.stream().map(buket -> {
                BrandEntity brandEntity = new BrandEntity();
                // 从桶中获取key，key就是brandId
                Long brandId = buket.getKeyAsNumber().longValue();
                brandEntity.setId(brandId);
                // 获取桶中子聚合
                Map<String, Aggregation> brandAggregationMap = buket.getAggregations().asMap();
                // 获取品牌名称的子聚合
    ...
    private SearchSourceBuilder buildDSL(SearchParam searchParam){
        ...
        java.lang.String keyword = searchParam.getKeyword();
        ...
        //1.2.3 规格参数过滤
        List<java.lang.String> props = searchParam.getProps();
        if (!CollectionUtils.isEmpty(props)){
            props.forEach(prop -> {
                java.lang.String[] attr = StringUtils.split(prop, ":");
                if (attr != null && attr.length == 2){
                    java.lang.String attrId = attr[0];
                    java.lang.String[] attrValues = StringUtils.split(attr[1], "-");
        ...
        if (sort != null){
            java.lang.String filed = "_score";
            SortOrder sortOrder = SortOrder.DESC;
            switch (sort){
                case 1: filed = "price"; break;
                case 2: filed = "price"; sortOrder=SortOrder.ASC; break;
                case 3: filed = "sales"; break;
                case 4: filed = "createTime"; break;
            }
